...
- **Revenue didn't drop because of fewer customers — it dropped because of smaller baskets.** *(Executive Overview)* The revenue-driver waterfall shows customer count cost −€9K and purchase frequency actually *helped* +€4K, but falling order value alone cost −€21K, netting the −€26K (−1.7%) YoY decline. The fix belongs in basket size, not acquisition or frequency.
- **One category's numbers don't add up — and that's worth flagging, not hiding.** *(Executive Overview)* The "Unknown" category shows +25.1% revenue growth alongside a −185.56% swing in gross margin YoY, an outlier far outside every other category's band (−1.4pp to +2.2pp). Given "Unknown" is the ghost-product/unmatched-reference bucket documented in [Data Cleaning & Quality](#data-cleaning--quality), this reads as a data-quality artifact, not a real margin collapse — worth investigating before it's ever presented as a business finding.
- **Customer churn is outpacing acquisition everywhere.** *(Customer Analysis)* In every month of H1 2024, new customers (104–140/month) run well below churned customers (197–254/month), and every single market retains far fewer customers than it loses — Nordic alone shows 802 retained vs. 1,674 churned. This is a shrinking-base problem, not a growth-rate problem.
- **Loyalty membership is a coin flip, not a lever — checked across 11 metrics, not just one.** *(Customer Analysis)* Members vs. non-members were compared on order value, purchase frequency, returns, discounting, margin, channel mix, marketing engagement, and churn — every single gap is 1–2%, with non-members marginally *ahead* on most. That's the signature of a randomly-assigned column, not a program with a real effect (full table in [powerbi/decisions_log.md](powerbi/decisions_log.md)). The deliberate decision was to **not build** a loyal-vs-non-loyal comparison visual at all — shipping one invites someone to explain a difference that doesn't exist.
- **Cancellations, not refunds, are the real leak — and getting the denominator wrong inverted that ranking.** *(Executive Overview)* An earlier version of this measure (dividing each loss type by a different, inconsistent base) named refunds the largest driver at €142K. Rebuilt on one consistent denominator (Gross Order Value) with mutually-exclusive, status-based numerators, the real ranking is Cancelled €115.6K (6.70%) > Returned €90.0K (5.21%) > Refunded €85.3K (4.95%) — refunds were overstated by ~67%. The recommended action (audit cancellations first) was already right; it's now actually supported by the data instead of contradicted by it.
- **Marketing budget is misallocated, by the report's own numbers.** *(Marketing Analysis)* Influencer has both the highest cost per acquisition (€28.21) and the lowest ROI (184.9%) of any channel, while Display and Pinterest deliver 6–8× better ROI (1,478.5% and 1,104.0%) at a fraction of the CPA. The dashboard's own recommended action is to shift spend from Influencer into Display/Pinterest.

Full detail: [04_EDA/insights.md](04_EDA/insights.md) (full 2021–2024 dataset findings) and the [Power BI Dashboard](#power-bi-dashboard) section above (H1 2024 vs. H1 2023 report findings) — the two cover different date scopes, see [Disclaimer](#disclaimer).
...
**Why this is the priority, not just a natural next step:** every segmentation variable tested so far in this report has failed to explain customer value — except one. Loyalty membership doesn't move repeat-purchase behavior (Key Business Insight above), and the underlying EDA already showed country, age group, and loyalty status barely move revenue per customer either. The one variable that *does* show a real, large effect is order frequency itself — the Customer Analysis's own chart shows revenue per customer climbing from €0.4K at 1 order to €2.1K at 5+ orders, a 5× spread. That's direct evidence an RFM-style framework (which is built on frequency and recency) will actually work as a targeting mechanism, unlike the variables already ruled out.
